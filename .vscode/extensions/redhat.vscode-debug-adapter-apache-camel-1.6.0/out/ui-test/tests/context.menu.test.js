...
(process.platform === 'darwin' ? describe.skip : describe)('Camel file context menu test', function () {
    ...
    it('Debug and Run menu item is available', async function () {
        const menu = await (0, utils_1.openContextMenu)(utils_1.CAMEL_ROUTE_YAML_WITH_SPACE);
        const runAndDebugMenuItem = await menu.hasItem(utils_1.CAMEL_RUN_DEBUG_ACTION_LABEL);
        (0, chai_1.expect)(runAndDebugMenuItem).to.not.undefined;
        await menu.close();
    });
    it('Run menu item is available', async function () {
        const menu = await (0, utils_1.openContextMenu)(utils_1.CAMEL_ROUTE_YAML_WITH_SPACE);
        const runItem = await menu.hasItem(utils_1.CAMEL_RUN_ACTION_LABEL);
        (0, chai_1.expect)(runItem).to.not.undefined;
        await menu.close();
    });
    it(`Execute command '${utils_1.CAMEL_RUN_ACTION_LABEL}' in context menu`, async function () {
        await (0, utils_1.selectContextMenuItem)(utils_1.CAMEL_RUN_ACTION_LABEL, await (0, utils_1.openContextMenu)(utils_1.CAMEL_ROUTE_YAML_WITH_SPACE));
        await (0, utils_1.waitUntilTerminalHasText)(driver, utils_1.TEST_ARRAY_RUN);
        await (0, utils_1.killTerminal)();
    });
    it(`Execute command '${utils_1.CAMEL_RUN_DEBUG_ACTION_LABEL}' in context menu`, async function () {
        if ((0, utils_1.isCamelVersionProductized)(process.env.CAMEL_VERSION)) {
            this.skip();
        }
        await (0, utils_1.selectContextMenuItem)(utils_1.CAMEL_RUN_DEBUG_ACTION_LABEL, await (0, utils_1.openContextMenu)(utils_1.CAMEL_ROUTE_YAML_WITH_SPACE));
